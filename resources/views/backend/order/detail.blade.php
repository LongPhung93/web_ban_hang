						...
						<div class="bootstrap-table">
							<div class="table-responsive">
								...
								<table class="table table-bordered" style="margin-top:20px;">
									...
									<tbody>
                                    @foreach($order->product_order as $key=>$value)
										<tr>
											<td>1</td>
											<td>
												<div class="row">
													<div class="col-md-4">
														<img width="100px" src="img/{{$value->img}}" class="thumbnail">
													</div>
													<div class="col-md-8">
														<p><b>Mã sản phẩm</b>: {{$value->sku}}</p>
														<p><b>Tên Sản phẩm</b>: {{$value->name}}</p>
														<p><b>Số lương</b> : {{$value->quantity}}</p>
													</div>
												</div>
											</td>
											<td>{{number_format($value->price,2,'.',',')}} VNĐ</td>
											<td>{{number_format($value->price*$value->quantity,2,'.',',')}} VNĐ</td>
                                    @endforeach
								...
								<table class="table">
									<thead>
										<tr>
											...
											<th>
												<h4 align='right' style="color: brown;">{{number_format($order->total,2,'.',',')}} VNĐ</h4>
											</th>
								...
								<div class="alert alert-primary" role="alert" align='right'>
									<a name="" id="" class="btn btn-success" onclick="return confirm('Đơn hàng sẽ được xử lý và tính vào doanh thu ?')" href="{{route('order.handle',$order->id)}}" role="button" style="width:100px;height:40px;">Xử lý</a>
								</div>
							</div>
						</div>
